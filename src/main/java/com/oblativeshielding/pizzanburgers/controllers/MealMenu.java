...
public class MealMenu extends GridPane {
    ...
    public MealMenu(MenuItemCategory[] categoryArray, RootScene root) {
        ...
        //nameBox.getChildren().add(new Label("Order Name"));
        this.add(new Label("Order Name:"), 0, 0);
        orderName = new TextField();
        //nameBox.getChildren().add(orderName);
        ...
        for(int i = 0; i < categories.length; i++) {
            ...
            for(int j = 0; j < c.max; j++) {
                // mini HBox for label-choice pair
                //HBox h = new HBox();
                ...
                choice.getItems().addAll(Arrays.asList(c.components));
                if(c.min < c.max) {
                    choice.getItems().add(null);
                }
                // add choicebox to list
                choices.get(i).add(choice);
                // add choicebox to hbox
    ...
    private void order() {
        ...
        for(int i = 0; i < categories.length; i++) {
            ...
            if(minCounter > 0) {
                System.out.println("Select " + minCounter + " more " + c.categoryName);
                return;
            }
        }
